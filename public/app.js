...
function render(data) {
  ...
  $("#owner-commission-body").innerHTML = commissions.length ? commissions.map((item) => `<tr><td><strong>${escapeHtml(item.name)}</strong></td><td>${escapeHtml(item.role)}</td><td>${escapeHtml(item.store)}</td><td>${isOwner ? `${Math.round((item.grossCommission || 0) * 100) / 100}` : ""}</td><td class="money">${moneyOrPending(item.commission, item.pending)}</td></tr>`).join("") : emptyRow(5, "暂无人员配置");
